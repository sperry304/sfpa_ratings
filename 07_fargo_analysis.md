...
for (i in 1:nrow(all_players)) {
  all_players$games_in_system[i] <- 
    count_games(all_players$player[i], results_no_forfeits)
}
```
...
Considering two players with ratings *π*<sub>*i*</sub> and *π*<sub>*j*</sub>, the probability of *i* winning the game can be calculated with the following:

$$\\text{P\[Player i wins\]} = \\frac{\\pi\_i}{\\pi\_i + \\pi\_j}$$
...
|  ranking| player                 | team                         |  rating|  games\_in\_system|
|--------:|:-----------------------|:-----------------------------|-------:|------------------:|
