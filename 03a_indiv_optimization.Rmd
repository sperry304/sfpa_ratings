...
for (i in 1:nrow(train)) {
  player1 <- train$home[i]
  player2 <- train$away[i]
  player1_rating <- get_rating(player1, elo_ratings)
  player2_rating <- get_rating(player2, elo_ratings)
  train$p1_start_rating[i] <- player1_rating
  train$p2_start_rating[i] <- player2_rating
  player1_expected <-
  ...
  train$p1_end_rating[i] <- player1_rating_new
  train$p2_end_rating[i] <- player2_rating_new
}
...
k_results <- map_dfr(seq(6, 40, by = 1), k_testing)
```
...
```{r}
# Weighted mean absolute error by rounded probability (to 1 digit)
k_results %>% 
