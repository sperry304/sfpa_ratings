...
date_loop <- 
  full_data %>% 
  select(match_date) %>% 
  filter(match_date > "2019-01-01") %>% 
  distinct() %>% 
  pull()

param_list <- list(
  as_of_match_date = rep(date_loop, 9), gamma_prior = rep(seq(2, 4, by = 0.25), each = 7), half_life_days = 1500, 
  games_to_robustness = 10
)
fargo_df <- pmap_dfr(param_list, run_fargo_through_date)

fargo_df %>% 
  group_by(match_date, gamma_prior) %>% 
...
saveRDS(fargo_df, "other_data/fargo_322.Rdata")
```
...
```{r}
win_prob_summary %>% 
  filter(gamma_prior > 2.5, gamma_prior < 3.5) %>% 
...
plot_multiple_player_ratings(player_list = team_members("Tandy Tokers"), fargo_df_latest)
plot_multiple_player_ratings(player_list = team_members("Route 101 Rawhides"), fargo_df_latest)
plot_multiple_player_ratings(player_list = team_members("Cafe Strikes Again"), fargo_df_latest)
plot_multiple_player_ratings(player_list = team_members("Lucky Horseshoe Caballeros"), fargo_df_latest)
plot_multiple_player_ratings(player_list = c("Jesse La Fear", "Nick Callado", "Stefano Lopez"), fargo_df_latest)
```
