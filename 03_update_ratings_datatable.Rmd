...
```{r}
player_of_interest <- "Skip Perry"
results_df <- results_no_forfeits
ratings_df <- fargo_ratings
a <- 3
```
...
player_list <- 
  bind_rows(
    results_df %>% transmute(player = home) %>% as_tibble(), 
    results_df %>% transmute(player = away) %>% as_tibble()
  ) %>% 
  distinct() %>% 
  arrange(player) %>% 
  lazy_dt()
...
player_list_with_game_count <- 
  player_list %>% 
  left_join(game_count, by = "player")
```
...
#collect_game_results(results_df, "Skip Perry")
...
get_updated_rating <- function(player_of_interest, collected_results_df, ratings_df, a) {
  #game_count <- 
  #  results_df %>% 
  #  filter(away == player_of_interest | home == player_of_interest) %>% 
  #  count() %>% 
...
results_to_ratings <- function(results_df, a, mae_stop = 100) {
  ...
  # This turns all of the games into season-player-opponent-result format
  collected_game_results <- 
    map_dfr(player_list, collect_game_results)
  ...
  while (abs_diff > mae_stop) {
    ...
    for (i in 1:nrow(fargo_ratings)) {
      ...
      fargo_ratings$rating[i] <- 
        get_updated_rating(
          player_name, 
          collected_results_df = collected_game_results, 
          ratings_df = fargo_ratings, 
          a = a
        )
    }
